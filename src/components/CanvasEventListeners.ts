...
export default function CanvasEventListeners() {
  ...
  // Beforeunload event
  useEffect(() => {
    const onBeforeUnload = (event: { preventDefault: () => void; returnValue: string }) => {
      event.preventDefault()
      // Esto es necesario en algunos navegadores para mostrar el diálogo de confirmación
      event.returnValue = '' // Algunos navegadores esperan esto, aunque esté marcado como deprecated.
    }
